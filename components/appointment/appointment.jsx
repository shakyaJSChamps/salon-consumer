...
const Appointments = () => {
  ...
  const filterAppointments = useCallback(() => {
    const filterByStatus = (appointment) =>
      !statusFilter ||
      appointment.status.toLowerCase() === statusFilter.toLowerCase();
    const filterByType = (appointment) =>
      !typeFilter ||
      appointment.services.some(
        (service) =>
          service.serviceName.toLowerCase() === typeFilter.toLowerCase()
      );
    ...
    const pendingFiltered = (appointments.pending || []).filter(
      (appointment) =>
        filterByStatus(appointment) &&
        filterByType(appointment) &&
        filterByDate(appointment)
    );
    const pastFiltered = (appointments.past || []).filter(
      (appointment) =>
        filterByStatus(appointment) &&
        filterByType(appointment) &&
        filterByDate(appointment)
  ...
  const containerStyle = {
    height:
      filteredPending.length === 0 && filteredPast.length === 0 ? "420px" : "",
  };
  const containerClass =
    filteredPending.length === 0 && filteredPast.length === 0
      ? "fixed_height"
      : "";
            ...
            {filteredPending.map((data, index) => (
              <div className={styles.upcomingSchedule} key={index}>
                ...
                <div className={styles.buttons}>
                  <button onClick={() => handelShowRescedule(data)}>
                    Re-Schedule
                  </button>
                  {data.status === "PENDING" && (
                ...
                {filteredPast.map((data, index) => (
                  <div key={index} className={styles.appointmentDiv}>
                    <div className={styles.pastSchedule} key={index}>
                      ...
                      <div
                        className={`${styles.upcomingDetails} upcomingDetailsPast`}
                      >
                        <h5>{data.salon.name}</h5>
                        <p>
                          <RxCalendar />
      ...
      {rescheduleShow && (
        <>
          <RescheduleAppointment
            ...
            handelShowAppointMent={handelShowAppointMent}
          />
        </>
      )}
      {ratingShow && (
